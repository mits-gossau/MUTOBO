	...
	$scope.findImageReference = function(context, alias) {
		...
		if (found) {
			...
			if (currentPage.variants) {
				currentPage.variants.forEach(function (variant) {
					console.log("test")
							props = contentEditingHelper.getAllProps(variant)
							if (props) {
								imageProperties = props.filter(function (prop) { return prop.alias.match("modules") })
								if (imageProperties[0].value.length >= 1) {
									console.log(imageProperties[0])
									console.log(imageProperties[0].value[0].image)

									imageReference = imageProperties[0].value[0].image
									console.log("test")
									found = true
